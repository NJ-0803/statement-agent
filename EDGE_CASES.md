...
| EC-22 | "Biggest expense" ambiguity | **PASS (fixed, then refined further)** | Added `sort_by`/`limit` to `search_transactions` (so "the single largest transaction" is answerable deterministically, not by the model eyeballing a list). Split into two prompt rules by how ambiguous the wording actually is: 2a — a clearly-worded question ("highest TRANSACTION") gets one direct, reasoned answer, closing with exactly one follow-up question offering the next useful cut (never a blocking pause — still a complete, self-contained answer); 2b — genuinely ambiguous wording ("biggest EXPENSE") still computes and presents multiple readings together, as originally built. Live-tested both, consistently across two different months. `tests/test_tools.py::TestSortAndLimit`, `DECISIONS.md` §15. |
